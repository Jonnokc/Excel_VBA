...
UserNameErr:
  ...
  ' ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  '
  ' For Each Project_Name In Range("Sources_List")
  '   Current_Source = Project_Name
  ...
  ' Loop through the sources and create the file for each source
    ...
    With NewBook
      ...
      "C:\Users\" & User_Name & "\Documents\" & Project_Name & "_" & "PCST_File"
    End With

    Windows(Project_Name & "_PCST_File.xlsx").Activate
    ...
    For i = 0 To UBound(Val_Wk_Array)
      ...
      ' SUB - Copies the data from Validation Form into new file
      Windows(Validation_File_Name).Activate
      Sheets(CurrentSheet).Select
      ActiveSheet.ListObjects(1).Range.AutoFilter Field:=5, _
      Criteria1:="<>"
  ...
  ' Criteria1:=Project_Name, Operator:=xlAnd
  ...
  For Each code In Range("Code_ID_List")
    ...
    If code = "72" Then
      ...
      With Sheets(Val_Wk_Array(0))
        ...
        ' Clears any extra filers
        If Sheets(Val_Wk_Array(0)).FilterMode Then
          Sheets(Val_Wk_Array(0)).ShowAllData
        End If
      ...
      With Sheets(Val_Wk_Array(1))
        ...
        ' Clears any extra filers
        If Sheets(Val_Wk_Array(1)).FilterMode Then
          Sheets(Val_Wk_Array(1)).ShowAllData
        End If
  ...
  Workbooks(Project_Name & "_PCST_File.xlsx").Close SaveChanges:=True
  Windows(Validation_File_Name).Activate
...
If ActiveWorkbook.Name = (Project_Name & "_PCST_File.xlsx") Then
  Workbooks(Project_Name & ".xlsx").Close SaveChanges:=False
End If
